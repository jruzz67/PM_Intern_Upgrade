...
const ApplyPage = () => {
    ...
    const [maxStepReached, setMaxStepReached] = useState(1); // Track the furthest step reached
                ...
                <div className="w-2/3 p-8">
                    ...
                    {currentStep === 1 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center relative cursor-pointer hover:border-orange-500">
                                ...
                                <input type="file" onChange={handleResumeUpload} className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"/>
                            </div>
                    ...
                    {currentStep === 2 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            ...
                            <textarea placeholder="Describe your key skills and relevant work experience..." className="w-full h-32 p-3 border rounded-md focus:ring-2 focus:ring-orange-500 outline-none mt-4"></textarea>
                        </motion.div>
                    )}
                    ...
                    {currentStep === 3 && (
                         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            ...
                            <textarea placeholder="List any specific achievements (e.g., awards, hackathon wins, project highlights)..." className="w-full h-48 p-3 border rounded-md focus:ring-2 focus:ring-orange-500 outline-none"></textarea>
                        </motion.div>
                    )}
                    ...
                    {currentStep === 4 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold mb-6">Step 4: Professional Profiles</h2>
                            <div className="space-y-4">
                                <div className="relative"><FiLink className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input placeholder="GitHub URL" className="w-full p-3 pl-10 border rounded-md"/></div>
                                <div className="relative"><FiLink className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input placeholder="LinkedIn URL" className="w-full p-3 pl-10 border rounded-md"/></div>
                                <div className="relative"><FiLink className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input placeholder="LeetCode/Portfolio URL" className="w-full p-3 pl-10 border rounded-md"/></div>
                            </div>
                        </motion.div>
                    )}
                    ...
                    <div className="flex justify-between mt-8">
                        <button onClick={() => setCurrentStep(s => Math.max(1, s - 1))} disabled={currentStep === 1} className="px-6 py-2 bg-gray-200 text-gray-700 font-semibold rounded-md disabled:opacity-50">Back</button>
                        {currentStep < 5 && <button onClick={handleNext} className="px-6 py-2 bg-orange-500 text-white font-semibold rounded-md hover:bg-orange-600">Next</button>}
                        {currentStep === 5 && <button onClick={handleSubmit} disabled={!termsAccepted} className="px-6 py-2 bg-green-500 text-white font-semibold rounded-md hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed">Submit Application</button>}
                    </div>
                </div>
            </div>
            )}
